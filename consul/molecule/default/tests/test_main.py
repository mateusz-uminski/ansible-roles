...
def test_consul_number_of_members_in_server_mode(host):
    num_servers = int(host.run("consul members | grep server | wc -l").stdout)
    assert num_servers == 1


def test_consul_cluster_has_been_bootstrapped(host):
    cmd = "consul operator raft list-peers"
    is_leader = host.run(cmd).succeeded
    assert is_leader
